...
do
    curl $url -o $out_dir/$file_name.csv
done < <( cat static_data/hdx_admin_pop_urls.json | jq -r --arg cnt $country_code '(.[] | select( .country_code | test( $cnt; "i" )) | .val | .[] | [ .download_url, .ma_file_name ]) | @csv' )